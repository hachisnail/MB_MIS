...
import {TransactionDescription} from "./ViewPageRenderer.jsx"
  ...
  return (
    <div
      className={`w-full h-[32rem] ${
        isLending ? "bg-[#E4E4E4]" : "bg-[#1D1911]"
      } rounded-r-4xl flex`}
    >
      {isLending && (
        <div
          className={`w-full max-w-10 h-full ${
            isLending ? "bg-white" : "bg-[#1D1911]"
          } rounded-r-4xl`}
        />
      )}
      ...
      <div
        className={`w-full h-full px-13 py-10 flex flex-col space-y-5 ${
          isLending ? "text-[#2F0000]" : "text-white"
        }`}
      >
