...
export const validateAppointmentSchedule = (appointmentData, existingEvents) => {
  ...
  if (startMinutes < sixAM || endMinutes > sixPM) {
    return {
      isValid: false,
      error: 'Please schedule your appointment between 6:00 AM and 6:00 PM'
    };
  }
  ...
  if (startMinutes >= endMinutes) {
    return {
      isValid: false,
      error: 'Please make sure your start time is before your end time'
    };
  }
  ...
  if (duration < 15) {
    return {
      isValid: false,
      error: 'Your appointment must be at least 15 minutes long'
    };
  }
  ...
  if (existingExclusiveEvent) {
    return {
      isValid: false,
      error: 'This time slot is reserved for a special event. Please choose another time.'
    };
  }

  // Check overlapping events limit (max 1)
 ...
 * Note: Schedules are NOT blocked by exclusive events, only by the 10-limit rule
...
export const validateScheduleCreation = (scheduleData, existingEvents) => {
  ...
  // Check overlapping events limit (max 10, including both schedules and appointments)
  const overlappingCount = countOverlappingEvents(
    existingEvents.filter(e => e.date === date),
    startTime,
    endTime
...
export const validateDateDisabling = (disableData, existingEvents) => {
  ...
  if (type === "day") {
    ...
    if (existingDisabledDay) {
      return {
        isValid: false,
        error: 'This date is already unavailable.',
        warning: null
      };
    }
...
export const checkTimeSlotAvailability = async (
  ...
  showToast('Checking available time slots...', 'info');
  ...
  try {
    ...
    todayAppointments.forEach(appointment => {
      ...
      if (status === 'APPROVED' && appointment.start_time && appointment.end_time) {
        ...
        if (timeSlots.includes(timeKey)) {
          ...
          // Also increment the count for this slot since we have an approved appointment
          counts[timeKey] += 1;
        }
      }
    });
    ...
    // For schedules: only blocked by confirmed slots and 10-limit rule
    const availableSlots = timeSlots.filter(slot => {
      ...
      return counts[slot] < 10; // Both types respect 10-limit rule
    });

    if (availableSlots.length === 0) {
      showToast(`No available time slots on this date`, 'warning');
    } else {
    ...
    showToast('Unable to check available time slots. Please try again.', 'error');
  } finally {
...
export const checkDateAvailability = async (date, axiosClient, checkType = 'appointment') => {
  ...
  try {
    ...
    for (const slot of timeSlots) {
      ...
      // Check if any approved appointment blocks this slot
      ...
      if (confirmedInSlot) {
        ...
        if (scheduleResponse.data && Array.isArray(scheduleResponse.data)) {
          ...
          // Check if slot count exceeds limit (10)
          if (slotCount >= 10) {
            isSlotAvailable = false;
          }
        }
