...
CREATE TABLE ContributionArtifacts (
    ...
    narrative TEXT,
    images JSON, -- [{ name: "file.jpg", size: 12345, type: "image/jpeg" }, ...]
    documents JSON, -- same as images
    related_images JSON, -- same as images
    image_urls JSON, -- ["https://..."]
    document_urls JSON, -- ["https://..."]
    related_image_urls JSON, -- ["https://..."]
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (contribution_id) REFERENCES Contributions(contribution_id) ON DELETE CASCADE
);
