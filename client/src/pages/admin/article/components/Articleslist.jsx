...
const Articleslist = ({ article, handleStatusChange, userRole, currentUserId, getStatusBadge, headers }) => {
  ...
  const onStatusChange = (value) => {
    if (value === "scheduled") {
      ...
      navigate(`/admin/article/edit-article/${encodedId}?mode=review`, {
        state: { forceReviewMode: true },
      });
      return; // do not call API here
    }
    if (value === "archived" && article.status !== "posted") {
      alert("Only posted items can be archived.");
      return;
    }
    handleStatusChange(article.article_id, value);
  };
    ...
    { key: "category", render: () => article.article_category },
    {
      ...
      render: () =>
        userRole === 1 ? (
          ...
          >
            {STATUS.map((s) => {
              const disabled = s.value === "archived" && article.status !== "posted";
              return (
